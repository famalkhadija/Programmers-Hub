...
export default function PrivateDiscussClient() {
 const { data: session } = useSession();
  const currentUser = session?.user?.email || session?.user?.id;
  const params = useParams();
  const group = params?.group;
const groupParam = decodeURIComponent(params.group);
const rawName = groupParam.split("_")[1]; 
const groupName = rawName
  ...
  const groupId = params.group;
    ...
    const owner = groupParam.split("_")[0]; // owner gmsil
  const [showInviteBox, setShowInviteBox] = useState(false);
  const [inviteBoxKey, setInviteBoxKey] = useState(0);

 const handleAddClick = () => {
 // Toggle logic
    setInviteBoxKey(prev => prev + 1); 
    setShowInviteBox(true);
  };
  const [messages, setMessages] = useState([]);
  const [newMessage, setNewMessage] = useState("");
  const messagesRef = collection(db, "privateGroups", group, "messages");
...
const currentEmail = session?.user?.email;
  ...
  const checkMembership = async () => {
    ...
    const groupRef = doc(db, "privateGroups", groupParam);
    ...
    const isMember = groupData.members?.includes(currentEmail);
  ...
  return;
    ...
    try {
      console.log("Replying to:", replyTo); // ✅ See if it's non-null here
...
const msg = {
  ...
  function Message({ msg, isMe }) {
    ...
    //close right click on outside click
    useEffect(() => {
    ...
    if (typeof window !== 'undefined' && isAllowed === null) {
  return null;
  }
if (isAllowed === false) {
  router.replace("/404");
  return null;
    ...
    return (
      <>
        ...
        <div
          ref={ref}
          id={`msg-${msg.id}`} //unique id for scroll target
  ...
  try {
    const res = await fetch("/api/private/delete", {
      method: "DELETE",
      headers: {
    ...
    if (res.ok) {
      alert("Group deleted successfully.");
          router.push("/privateDiscussions");
  ...
  return (
    <div className="flex h-full w-full ">
      <PrivateStickyBar />
      <div className="  h-[calc(100vh-16vh)] xl:w-[79%] lg:w-[75%] md:w-[70%] w-[calc(100%-43px)]">
        <div className=" flex justify-between items-center dark:border-b dark:border-blue-700  md:dark:border md:dark:border-blue-700 pt-[6px] dark:bg-black dark:text-white">
          <h1 className="md:text-xl text-lg px-4 font-bold">
     ...
     <button
             onClick={handleAddClick}
              type="button"
              className={`${owner!==currentUser?'h-10':''} text-white bg-gradient-to-r from-cyan-500 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-cyan-300 dark:focus:ring-cyan-800 font-bold rounded-lg text-sm md:px-5 text-center me-2 mb-2 shadow-[1px_1px_4px_gray] hover:shadow-[1px_1px_8px_gray] md:w-fit w-28 `}
            >
              Add members
            </button>
            
                  {showInviteBox && (
        <InviteLinkBox key={inviteBoxKey} groupId={groupId} />
      )}

                {(owner==currentUser)&&<button
      onClick={() => handleDeleteGroup(groupParam)}
      className=" text-white bg-gradient-to-r from-cyan-500 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-cyan-300 dark:focus:ring-cyan-800 font-bold rounded-lg text-sm md:px-5  py-2.5 text-center me-2 mb-2 shadow-[1px_1px_4px_gray] hover:shadow-[1px_1px_8px_gray] md:w-fit w-24"
      title="Delete"
    >
      Delete chat
    </button>}
