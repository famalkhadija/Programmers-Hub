  ...
  if (session) {
    return (
      ...
      <div className="mx-auto my-10 md:my-24">
        <div className="text-center">Signed in as {session.user.email} </div> <br />
        <div className="text-center"><button className=" dark:hover:bg-black dark:border-none px-4 py-2 border rounded-md text-sm hover:bg-slate-50" onClick={() => signOut()}>Sign Out</button></div>
        </div>
      </>
    );
  }
